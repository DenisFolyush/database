...
class User(db.Model):
    ...
    # Видаляємо дублюючий backref
    cities = db.relationship('City', secondary='user_has_city', lazy=True)
...
# Модель для таблиці з'єднання між User і City
class UserHasCity(db.Model):
    __tablename__ = 'user_has_city'
    
    # Зв'язки з іншими таблицями
    user_id_user = db.Column(db.Integer, db.ForeignKey('user.id_user'), primary_key=True)
    city_id_city = db.Column(db.Integer, db.ForeignKey('city.id_city'), primary_key=True)
    
    user = db.relationship('User', backref=db.backref('user_has_city', lazy=True))
    city = db.relationship('City', backref=db.backref('user_has_city', lazy=True))
    
    def __repr__(self):
        return f"<UserHasCity(user_id={self.user_id_user}, city_id={self.city_id_city})>"
