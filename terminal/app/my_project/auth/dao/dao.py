...
from app import db
from app.my_project.auth.domain.models import Country, City, WeatherConditionLabel, CityWaterTemperature, DailyWeather, HourlyWeather, WeatherAlert, WeatherForecast,UserHasCity ,User, UserHasCity, UserPreferences,UserHasCity
...
class UserPreferencesDAO:
    ...
    @staticmethod
    def delete(id_user_preferences):
        preference = UserPreferences.query.get(id_user_preferences)
        if preference:
            db.session.delete(preference)
            db.session.commit()
        else:
            raise ValueError("User preference not found")

class UserHasCityDAO:
    @staticmethod
    def get_all_user_city_links():
        query = (
            db.session.query(
                User.id_user.label("id_user"),
                User.name.label("user_name"),
                City.id_city.label("id_city"),
                City.name.label("city_name")
            )
            .join(UserHasCity, User.id_user == UserHasCity.user_id_user)
            .join(City, City.id_city == UserHasCity.city_id_city)
        )
        return query.all()
